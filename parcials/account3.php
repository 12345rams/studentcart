...
$showError = "false";
    if($_SERVER["REQUEST_METHOD"] == "POST"){
       ...
       $about=$_POST['about'];
         ...
         $watsappno=$_POST['watsappno'];
      
   
       $sql=" UPDATE `signups` SET `nameofproductpriceof`='$name',`priceofproduct`='$accountid',`Detailsofproduct`='$about',`Mobile no`='$mobileno',`Watsapp no`='$watsappno' WHERE email='$email'";
                $result = mysqli_query($conn, $sql);
                if($result){
                    $showAlert = true;
                    echo'Successfully uploading';
                    header("Location: /ram/parcials/upload4.php");
                    exit();
                       }
                      else 
...
<input type="text" name="name">
    ...
    <label for="">Watsapp No</label>
    <input type="text" name="watsappno">
